...
export function StudentRoster() {
  ...
  return <main className="min-h-screen bg-[#eef5fb] text-slate-900"><div className="grid min-h-screen lg:grid-cols-[240px_minmax(0,1fr)]">
    <div className="print:hidden"><Sidebar /></div>
    <div className="p-5"><div className="mx-auto max-w-5xl rounded-[28px] bg-white p-7 shadow-sm">
    <div className="flex flex-wrap items-center justify-between gap-4"><div><h1 className="text-3xl font-black">Estudiantes · {classItem?.title ?? "Clase"}</h1><p className="mt-2 text-slate-500">Cuentas nuevas administradas por el docente. El historial anterior no se muestra aquí.</p></div><button className="print:hidden rounded-xl border px-4 py-2 font-bold" onClick={() => window.print()}><Printer className="mr-2 inline h-4 w-4"/>Imprimir credenciales</button></div>
