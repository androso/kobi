...
The runner validates raw history before applying any Drizzle migration, then holds a PostgreSQL advisory lock across the complete schema transition so concurrent deploys wait instead of changing the schema in parallel. Raw migrations are stored in filename order under `migrations/`; each file runs once in its own transaction and is recorded in `public.kobi_raw_migrations` with its SHA-256 checksum, application time, duration, and release identifier. That history table is private to the migration connection: `anon`, `authenticated`, and `public` receive no table privileges.

Committed migration files are immutable, applied files cannot be deleted or renamed, and new files must sort after the highest applied file. If any of those checks fail, the runner stops before Drizzle or raw SQL changes run; restore the original file or add a new higher-numbered migration. On the first tracked deployment to an existing database, the runner baselines the committed raw files after confirming pre-existing Drizzle history, so it does not replay legacy SQL; a clean database has no pre-existing Drizzle history and runs every raw file normally. A migration that cannot run in a transaction, such as `create index concurrently`, must be delivered as a separately reviewed operational change and documented with its retry and recovery procedure rather than added to this directory.
