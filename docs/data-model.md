...
Schema-as-code + migrations: `packages/db/src/schema` (Drizzle ORM, postgres.js driver). `pnpm --filter @kobi/db run db:generate` regenerates SQL from the schema into `packages/db/drizzle/`; `pnpm --filter @kobi/db run db:migrate` applies generated migrations and then each pending custom migration under `packages/db/migrations`. Custom migrations cover extensions, indexes, constraints, policies, and RPCs that are not expressible as Drizzle schema. They are serialized by a PostgreSQL advisory lock and tracked with immutable SHA-256 checksums in `kobi_raw_migrations`; see [`database-deployments.md`](database-deployments.md) for release ordering and recovery rules.
