...
That decision is the **checkpoint gate**: coherent context can enqueue `apps/worker/src/jobs/evaluateCheckpoint.job.ts` immediately after a new `lesson_state`, while `checkpointScheduler.job.ts` provides a pg-boss timer fallback (default every `CHECKPOINT_INTERVAL_MINUTES` = 2 min per active session). A retrying `finalize-session` job forces a final evaluation after class close once transcription and lesson-state progress settle. The OpenAI-backed checkpoint agent (`apps/worker/src/checkpoint/evaluateCheckpoint.ts`) still decides whether the accumulated material is sufficient and valid — see `docs/contracts.md` §2 for the decision shape. Only when the agent says `ready: true` does the job call `retrieveCurriculumMatches()` and hand off to Area C's `generate-activity-artifacts`.
