...
Set `RETENTION_ADMIN_TOKEN` to a random value of at least 32 characters, then call `DELETE /api/sessions/:id/classroom-data` with `Authorization: Bearer <token>`. It first marks the session as deletion-requested and waits for queued or running transcription work to quiesce, then deletes that session's raw audio and transcript text, removes lesson-state and checkpoint rows, and redacts candidate context snapshots through the same audited service as scheduled cleanup. Aggregate session, assignment, and event records remain preserved.
