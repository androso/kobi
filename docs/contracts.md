...
Produced by the checkpoint agent (`apps/worker/src/checkpoint/evaluateCheckpoint.ts`, OpenAI structured output) and consumed by `apps/worker/src/jobs/evaluateCheckpoint.job.ts`. A cheap context-readiness gate wakes this semantic checkpoint as soon as at least two confident, topic-stable lesson-state snapshots have enough objective/vocabulary grounding. The independent timer (`checkpointScheduler.job.ts`, default every `CHECKPOINT_INTERVAL_MINUTES` = 2 min) remains a fallback. Session close enqueues a retrying server-owned finalizer that waits for transcript/lesson-state progress and forces one final checkpoint over the accumulated lesson.
...
- Repository-reused, deterministic static-fallback, adapted, and model-generated artifacts all use this same runnable contract. Production runtime verification executes generated code in a zero-egress ephemeral Modal/gVisor sandbox, feeds failures into bounded model repair, and reruns the same gate before persistence. When `OPENAI_ACTIVITY_GENERATION_MODE=model_only`, generation failure is surfaced as no candidates and existing non-model candidates are not treated as current; no deterministic activity is substituted. Static fallback artifacts must remain interactive; there is no manifest-only renderer fallback.
